...
public class GetDVDocumentoApiDAOImpl extends DaoBaseImpl<DocumentoDVDataPK, DocumentoDVDataJPA> implements GetDVDocumentoApiDAO{
		...
		try {	
			...
			if (listado != null && !listado.isEmpty()) {			
				for (Object[] tmp : listado) {	
					...
					if (tmp[23] != null) outDVDocumentoDTO.setTxtInfoREA(String.valueOf(tmp[23]));	
					if (tmp[24] != null) outDVDocumentoDTO.setTxtInfoLPC(String.valueOf(tmp[24]));
					if (tmp[25] != null) outDVDocumentoDTO.setTxtInfoGeneral(String.valueOf(tmp[25]));
	...
	private List<OutputDeclaracionesDeValorDocLinDTO> getDatosLineas(InputDeclaracionesDeValorDocumentoDTO input) {
		...
		try {	
			...
			// Ver separadore decimales 
			String separadores = ",.";
			String sepDecimal = "," ;
			
			if ("csv".equals(input.getTipoDocumento())) {
				separadores = ".,";
				sepDecimal = "." ;
			}
			/* replace en java "|; del producto y de la marca */
			/* en los numeros cambiar separadores decimales y de mil segun tipo informe , quitar la coma si no hay nada a la derecha de esta */
			...
			select.append(",L.COD_N_MERCA CODIGO ");
			...
			select.append(",L.NUM_ANYO ANYO ");
			select.append(",L.COD_N_VERSION VERSION_N ");
			select.append(",L.COD_N_TARIC CODIGO ");
			select.append(",'' PRODUCTO ");
			select.append(",'' MARCA ");
			select.append(",L.COD_N_TARIC CODIGO_TARIC ");
			...
			if (listado != null && !listado.isEmpty()) {			
				for (Object[] tmp : listado) {	
					...
					if (tmp[3] != null) outputDVDocLinDTO.setCodigoProducto(String.valueOf(tmp[3]));
					if (tmp[4] != null) outputDVDocLinDTO.setNombreProducto(replaceSpecialChars(String.valueOf(tmp[4])));
					if (tmp[5] != null) outputDVDocLinDTO.setMarca(replaceSpecialChars(String.valueOf(tmp[5])));
					if (tmp[6] != null) outputDVDocLinDTO.setCodigoTaric(String.valueOf(tmp[6]));
					outputDVDocLinDTO.setTipoLinea(String.valueOf(tmp[7]));
					if (tmp[8] != null) outputDVDocLinDTO.setCodigoRea(String.valueOf(tmp[8]));
					if (tmp[9] != null) outputDVDocLinDTO.setPaisOrigen(String.valueOf(tmp[9]));
					if (tmp[10] != null) outputDVDocLinDTO.setLpc(String.valueOf(tmp[10]));
					outputDVDocLinDTO.setNumeroBultos(formatNumber(input.getTipoDocumento(),String.valueOf(tmp[11])));
					if (tmp[12] != null) outputDVDocLinDTO.setTipoBultos(String.valueOf(tmp[12]));
					...
					if (tmp[16] != null) outputDVDocLinDTO.setVolumen(formatNumber(input.getTipoDocumento(),String.valueOf(tmp[16])));
					if (tmp[17] != null) outputDVDocLinDTO.setAlcohol(formatNumber(input.getTipoDocumento(),String.valueOf(tmp[17])));
					if (tmp[18] != null) outputDVDocLinDTO.setPlato(formatNumber(input.getTipoDocumento(),String.valueOf(tmp[18])));
					if (tmp[19] != null) outputDVDocLinDTO.setPrecio(formatNumber(input.getTipoDocumento(),String.valueOf(tmp[19])));
