package es.mercadona.gesaduan.dao.declaracionesdevalorapi.getvddocument.v1.impl;
...
@Stateless
public class GetVDDocumentApiDAOImpl extends DaoBaseImpl<DocumentoDVDataPK, DocumentoDVDataJPA> implements GetVDDocumentApiDAO{
	...
	private OutputDeclaracionesDeValorDocCabDTO getDatosCabecera(InputValueDeclarationDocumentDTO input) {
		...
		try {	
			...
			select.append(",DECODE(D.COD_N_PROVEEDOR, NULL, REGEXP_REPLACE(BL.TXT_NOMBRE, '\"|;', ''), REGEXP_REPLACE(P.TXT_RAZON_SOCIAL, '\"|;', '')) NOMBRE_ORIGEN ");
			...
			select.append(",DECODE(D.COD_N_PROVEEDOR,NULL,REGEXP_REPLACE(PU.TXT_NOMBRE_PUERTO, '\"|;', ''),REGEXP_REPLACE(C.TXT_RAZON_SOCIAL, '\"|;', '')) IMP_NOMBRE ");
			select.append(",DECODE(D.COD_N_PROVEEDOR,NULL,REGEXP_REPLACE(PU.TXT_DIRECCION, '\\\"|;', ''),REGEXP_REPLACE(C.TXT_DIRECCION, '\"|;', '')) IMP_DIRECCION ");
			select.append(",DECODE(D.COD_N_PROVEEDOR,NULL,'',C.TXT_COD_POSTAL) IMP_CP ");
			select.append(",DECODE(D.COD_N_PROVEEDOR,NULL,'',C.TXT_LOCALIDAD) IMP_POBLACION ");
			select.append(",DECODE(D.COD_N_PROVEEDOR,NULL,'',C.TXT_PROVINCIA) IMP_PROVINCIA ");
			...
			select.append(",D.COD_N_PROVEEDOR COD_PROVEEDOR ");	
