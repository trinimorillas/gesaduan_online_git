...
public class GetAlertasDAOImpl extends BaseDAO<AlertasJPA> implements GetAlertasDAO{
	...
	public OutputAlertasDTO getAlertas(InputAlertasDTO input, BoPage paginacion) {
		...
		try {
			...
			String subQueryUnion2 = "SELECT DISTINCT " +
					...
					"INNER JOIN D_TIPO_ELEMENTO TIPO ON TIPO.COD_V_TIPO_ELEMENTO = AL.COD_V_TIPO_ELEMENTO " + 
					"INNER JOIN O_DECLARACION_VALOR_CAB DV ON DV.COD_V_EXPEDICION = EXPAL.COD_V_EXPEDICION " + 
					"      AND DV.COD_N_DECLARACION_VALOR = EXPAL.COD_N_DECLARACION_VALOR "	+ 
			...
			if (listado != null && !listado.isEmpty()) {
				for (Object[] tmp : listado) {
					...
					if(esExpedicion.equals("S")) {
						...
						receptores = getReceptoresExped(codigoAlertaParse, codigoElemento, codigoExpedicion, fechaAlbaran);
	...
	private List<ReceptoresAlertasDTO> getReceptoresExped(Long codAlerta, String codigoElemento, String codigoExpedicion, String fechaAlbaran){
		...
		try {
			...
			sql.append(" INNER JOIN S_USUARIO_NOTIF_ALERTA_EXPED UNAE ON (UNAE.COD_N_ALERTA = NOTIF.COD_N_ALERTA ");
			...
			sql.append(" WHERE NOTIF.COD_N_ALERTA = ?codAlerta ");
			...
			sql.append(" FROM S_NOTIF_ALERTA_EXPED_DV NOTIF ");
			sql.append(" INNER JOIN S_CONTACTO_NOTIF_ALERTA_EXPED CNAE ON (CNAE.COD_N_ALERTA = NOTIF.COD_N_ALERTA ");
