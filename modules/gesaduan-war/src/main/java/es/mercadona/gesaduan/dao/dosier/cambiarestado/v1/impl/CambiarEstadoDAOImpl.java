...
public class CambiarEstadoDAOImpl extends DaoBaseImpl<Long, DosierJPA> implements CambiarEstadoDAO {
	...
	private final String CLASS_NAME = "CambiarEstadoDAOImpl(GESADUAN)";
	...
	public void actualizaContenedores(DosierJPA dosierJPA) {
		...
		try {
			...
			// Si la carga pasa de no validada a no asignada se inicializan los campos de
			// restantes
			...
			this.logger.error(Constantes.FORMATO_ERROR_LOG, CLASS_NAME, "actualizaContenedores",
	...
	public void actualizaEquipos(DosierJPA dosierJPA) {
		...
		try {
			// Si se invalida el único dosier del equipo, el estado de documentación del
			// equipo pasará a "Pendiente"
			...
			// Si alguno de los contenedores del equipo se encontraba asociado a otro
			// dosier, el estado de documentación del equipo pasará a "En curso"
			...
			this.logger.error("({}-{}) ERROR - {} {}", CLASS_NAME, "actualizaEquipos", e.getClass().getSimpleName(),
					e.getMessage());
			throw new ApplicationException(e.getMessage());
		}
	}
	...
	public void eliminaRelacionEquipo(DosierJPA dosierJPA) {
		...
		try {
			...
			this.logger.error("({}-{}) ERROR - {} {}", CLASS_NAME, "eliminaRelacionEquipo",
					e.getClass().getSimpleName(), e.getMessage());
			throw new ApplicationException(e.getMessage());
		}
	...
	public OutputCambiarEstadoDTO cambiarEstado(DosierJPA dosierJPA) {
		...
		try {
			...
			this.logger.error("({}-{}) ERROR - {} {}", CLASS_NAME, "actualizaContenedores",
					e.getClass().getSimpleName(), e.getMessage());
			throw new ApplicationException(e.getMessage());
		}
