...
public class PutDosierDAOImpl extends DaoBaseImpl<Long, DosierJPA> implements PutDosierDAO {
	...
	public void crearFacturas(DosierEquipoJPA dosierEquipo) {		
		try {			
			...
			sql.append("COD_V_APLICACION, COD_V_USUARIO_CREACION, COD_N_BLOQUE_LOGISTICO, COD_N_PUERTO_DESEMBARQUE, COD_N_CATEGORIA, NUM_DOSIER, NUM_ANYO_DOSIER) ");
			...
			sql.append("COD_N_BLOQUE_LOGISTICO, COD_N_PUERTO_DESEMBARQUE, COD_N_CATEGORIA, NUM_DOSIER, NUM_ANYO ");
			sql.append("FROM (SELECT BL.COD_N_PROVINCIA, PE.COD_N_BLOQUE_ORIGEN AS COD_N_BLOQUE_LOGISTICO, PE.COD_N_PUERTO_DESEMBARQUE, C.COD_N_CATEGORIA, CE.NUM_DOSIER, CE.NUM_ANYO, ");
			...
			sql.append("INNER JOIN D_CARGA C ON (C.COD_V_ALMACEN_ORIGEN = CE.COD_V_ALMACEN AND C.COD_V_CARGA = CE.COD_V_CARGA AND C.COD_N_TIPO_CARGA IN (1, 2)) ");
			sql.append("WHERE CE.NUM_DOSIER = ?numDosier AND CE.NUM_ANYO = ?anyoDosier AND NOT EXISTS (");
			sql.append("SELECT 1 FROM O_DECLARACION_VALOR_CAB DV WHERE NVL(DV.COD_N_CATEGORIA,0) = NVL(C.COD_N_CATEGORIA, 0) AND DV.NUM_DOSIER = CE.NUM_DOSIER ");
			sql.append("AND DV.NUM_ANYO_DOSIER = CE.NUM_ANYO) ");
			sql.append("GROUP BY CE.NUM_ANYO, BL.COD_N_PROVINCIA, PE.COD_N_BLOQUE_ORIGEN, PE.COD_N_PUERTO_DESEMBARQUE, C.COD_N_CATEGORIA, CE.NUM_DOSIER, CE.NUM_ANYO) ");
	...
	public void insertarLineasFacturas(DosierEquipoJPA dosierEquipo) {		
		try {			
			...
			sql.append("ROW_NUMBER() OVER (PARTITION BY COD_N_DECLARACION_VALOR,NUM_ANYO,COD_N_VERSION,COD_N_PRODUCTO ORDER BY COD_N_PRODUCTO ASC) AS PRIMERO ");
			...
			sql.append("(E.NUM_CANTIDAD_EXPEDIDO_PESO/1000)*TO_NUMBER('0,9') AS NUM_PESO_NETO,E.NUM_CANTIDAD_EXPEDIDO_PESO/1000 AS NUM_PESO_BRUTO, ");
			sql.append("DECODE(P.TXT_UNIDAD_MEDIDA,'ML',P.NUM_FORMATO_VENTA/1000,'LI',P.NUM_FORMATO_VENTA) AS NUM_VOLUMEN, ");
			sql.append("DECODE(E.COD_V_TIPO_CANTIDAD,'P',E.NUM_CANTIDAD_EXPEDIDO_PESO,'C',E.NUM_CANTIDAD_EXPEDIDO_UNIDAD) AS NUM_CANTIDAD, ");
			sql.append("ROUND(ROUND(DECODE(E.COD_V_TIPO_CANTIDAD,'P',(ROUND(E.NUM_CANTIDAD_EXPEDIDO_PESO/1000,2))*PCA.NUM_PCA, ");
			sql.append("E.NUM_CANTIDAD_EXPEDIDO_UNIDAD*PCA.NUM_PCA),2)/DECODE(E.COD_V_TIPO_CANTIDAD,'P',E.NUM_CANTIDAD_EXPEDIDO_PESO,'C', ");
			sql.append("E.NUM_CANTIDAD_EXPEDIDO_UNIDAD),'4') AS NUM_PRECIO_UNIDAD, ");
