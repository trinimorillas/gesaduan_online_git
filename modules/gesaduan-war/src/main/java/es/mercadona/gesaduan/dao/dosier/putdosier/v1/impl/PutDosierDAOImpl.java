...
public class PutDosierDAOImpl extends DaoBaseImpl<Long, DosierJPA> implements PutDosierDAO {
	...
	public void insertarLineasFacturas(DosierJPA dosierJPA) {		
		try {			
			...
			sql.append("INSERT INTO O_DECLARACION_VALOR_LIN (COD_N_DECLARACION_VALOR,NUM_ANYO,COD_N_VERSION,NUM_LINEA,COD_N_MERCA,COD_N_TARIC,COD_V_REA, ");
			...
			sql.append("SELECT COD_N_DECLARACION_VALOR,NUM_ANYO,COD_N_VERSION,NUM_LINEA,COD_N_PRODUCTO,COD_N_TARIC,COD_V_REA,TXT_NOMBRE_PRODUCTO,TXT_FORMATO, ");
			...
			sql.append("FROM ");
			sql.append("( ");
			sql.append("SELECT COD_N_DECLARACION_VALOR,NUM_ANYO,COD_N_VERSION, ");
			sql.append("ROW_NUMBER() OVER(PARTITION BY COD_N_DECLARACION_VALOR,NUM_ANYO,COD_N_VERSION ORDER BY COD_N_DECLARACION_VALOR) NUM_LINEA, ");
			sql.append("COD_N_PRODUCTO,COD_N_TARIC,COD_V_REA,TXT_NOMBRE_PRODUCTO,TXT_FORMATO, ");
			sql.append("TXT_MARCA,NOMBRE_BULTO,NUM_BULTOS,NUM_PESO_NETO,NUM_PESO_BRUTO,NUM_VOLUMEN,NUM_CANTIDAD,NUM_PRECIO_UNIDAD,NUM_IMPORTE_TOTAL, ");
			...
			sql.append("COD_N_PRODUCTO,NOMBRE_BULTO) AS NUM_BULTOS, ");
			sql.append("SUM(NUM_PESO_NETO) OVER (PARTITION BY COD_N_DECLARACION_VALOR,NUM_ANYO,COD_N_VERSION,COD_N_PRODUCTO,NOMBRE_BULTO) AS NUM_PESO_NETO, ");
			sql.append("SUM(NUM_PESO_BRUTO) OVER (PARTITION BY COD_N_DECLARACION_VALOR,NUM_ANYO,COD_N_VERSION,COD_N_PRODUCTO,NOMBRE_BULTO) AS NUM_PESO_BRUTO, ");
			sql.append("SUM(NUM_VOLUMEN) OVER (PARTITION BY COD_N_DECLARACION_VALOR,NUM_ANYO,COD_N_VERSION,COD_N_PRODUCTO,NOMBRE_BULTO) AS NUM_VOLUMEN, ");
			sql.append("SUM(NUM_CANTIDAD) OVER (PARTITION BY COD_N_DECLARACION_VALOR,NUM_ANYO,COD_N_VERSION,COD_N_PRODUCTO,NOMBRE_BULTO) AS NUM_CANTIDAD, ");
			sql.append("NUM_PRECIO_UNIDAD,ROUND(SUM(NUM_IMPORTE_TOTAL) OVER (PARTITION BY COD_N_DECLARACION_VALOR,NUM_ANYO,COD_N_VERSION, ");
			sql.append("COD_N_PRODUCTO,NOMBRE_BULTO),2) AS NUM_IMPORTE_TOTAL, ");
			sql.append("NUM_GRADO_ALCOHOL,NUM_GRADO_PLATO,COD_V_PAIS,MCA_PRODUCTO_LPC, ");
			sql.append("ROW_NUMBER() OVER (PARTITION BY COD_N_DECLARACION_VALOR,NUM_ANYO,COD_N_VERSION,COD_N_PRODUCTO,NOMBRE_BULTO ORDER BY COD_N_PRODUCTO ASC) ");
			...
			sql.append("DECODE(P.NUM_PESO_NETO_ALTERNATIVO,NULL, ");
			sql.append("(E.NUM_CANTIDAD_EXPEDIDO_PESO/1000)*TO_NUMBER('0,9'), ");
			sql.append("DECODE(E.COD_V_TIPO_CANTIDAD,'P',E.NUM_CANTIDAD_EXPEDIDO_PESO,'C',E.NUM_CANTIDAD_EXPEDIDO_UNIDAD) ");
			sql.append("*(P.NUM_PESO_NETO_ALTERNATIVO/1000)) AS NUM_PESO_NETO, ");
			...
			sql.append("ROUND(DECODE(E.COD_V_TIPO_CANTIDAD,'P',(ROUND(E.NUM_CANTIDAD_EXPEDIDO_PESO/1000,4))*PCA.NUM_PCA, ");
			sql.append("E.NUM_CANTIDAD_EXPEDIDO_UNIDAD*PCA.NUM_PCA),4) AS NUM_IMPORTE_TOTAL, ");
			...
			sql.append("AND TP.FEC_D_INICIO <= TRUNC(SYSDATE) ");
			sql.append("AND (TP.FEC_D_FIN IS NULL OR TP.FEC_D_FIN >= TRUNC(SYSDATE))) ");
	...
	public void modificarPesoBruto(DosierJPA dosierJPA) {		
		try {			
			...
			sql.append("UPDATE O_DECLARACION_VALOR_LIN ");
			sql.append("SET NUM_PESO_BRUTO = NUM_PESO_NETO*TO_NUMBER('1,1') ");
			sql.append("WHERE (COD_N_DECLARACION_VALOR,NUM_ANYO,COD_N_VERSION,COD_N_MERCA,TXT_NOMBRE_BULTO_DV) IN ( ");
			sql.append("SELECT DVL.COD_N_DECLARACION_VALOR,DVL.NUM_ANYO,DVL.COD_N_VERSION,DVL.COD_N_MERCA,DVL.TXT_NOMBRE_BULTO_DV ");
			sql.append("FROM O_DECLARACION_VALOR_LIN DVL ");
			sql.append("INNER JOIN O_DECLARACION_VALOR_CAB DVC ON (DVC.COD_N_DECLARACION_VALOR= DVL.COD_N_DECLARACION_VALOR ");
			sql.append("AND DVC.NUM_ANYO= DVL.NUM_ANYO ");
			sql.append("AND DVC.COD_N_VERSION= DVL.COD_N_VERSION) ");
			sql.append("WHERE DVC.NUM_DOSIER = ?numDosier ");
			sql.append("AND DVC.NUM_ANYO_DOSIER = ?anyoDosier ");
			sql.append("AND DVL.NUM_PESO_NETO >= DVL.NUM_PESO_BRUTO)");
			...
			this.logger.error(Constantes.FORMATO_ERROR_LOG, NOMBRE_CLASE, "modificarPesoBruto", ex.getClass().getSimpleName(), ex.getMessage());	
			throw new ApplicationException(ex.getMessage());			
		}	
	}
	...
	public void validarPaisREA(DosierPkJPA dosier, String codigoUsuario) {		
		try {			
			...
			sql.append("AND TP.FEC_D_INICIO <= TRUNC(SYSDATE) AND (TP.FEC_D_FIN IS NULL OR  TP.FEC_D_FIN >= TRUNC(SYSDATE))) ");
	...
	public void validarPrecioProducto(DosierPkJPA dosier, String codigoUsuario) {		
		try {			
			...
			sql.append("'N', 'N', 'N', SYSDATE, 'GESADUAN', ?codigoUsuario ");
			sql.append("FROM ( ");
			sql.append("SELECT DVL.COD_N_MERCA, DV.COD_N_DECLARACION_VALOR, DV.NUM_ANYO, DV.COD_N_VERSION,OE.COD_V_ALMACEN, ");
			sql.append("ROW_NUMBER() OVER( ");
			sql.append("PARTITION BY DVL.COD_N_MERCA,OE.COD_V_ALMACEN,DV.COD_N_DECLARACION_VALOR,DV.NUM_ANYO,DV.COD_N_VERSION ");
			sql.append("ORDER BY DVL.COD_N_MERCA,OE.COD_V_ALMACEN) NUMERO ");
			sql.append("FROM O_DECLARACION_VALOR_CAB DV ");
			sql.append("INNER JOIN O_DECLARACION_VALOR_LIN DVL ON (DVL.COD_N_DECLARACION_VALOR = DV.COD_N_DECLARACION_VALOR ");
			sql.append("AND DVL.NUM_ANYO = DV.NUM_ANYO AND DVL.COD_N_VERSION = DV.COD_N_VERSION) ");
			sql.append("INNER JOIN S_DECLARACION_VALOR_PEDIDO DVP ON (DVP.COD_N_DECLARACION_VALOR = DV.COD_N_DECLARACION_VALOR "); 
			sql.append("AND DVP.NUM_ANYO_DV = DV.NUM_ANYO AND DVP.COD_N_VERSION_DV = DV.COD_N_VERSION) ");
			sql.append("INNER JOIN O_EXPEDIDO OE ON (OE.COD_V_PEDIDO = DVP.COD_V_PEDIDO AND OE.COD_N_PRODUCTO = DVL.COD_N_MERCA)  ");			
			sql.append("LEFT JOIN S_PCA_PRODUCTO PCA ON (PCA.COD_N_PRODUCTO = DVL.COD_N_MERCA AND PCA.COD_V_CENTRO = OE.COD_V_ALMACEN) ");
			sql.append("WHERE DV.COD_V_EXPEDICION IS NULL ");
			...
			sql.append("SELECT 1 ");
			sql.append("FROM S_NOTIF_ALERTA_EXPED_DV NA ");
			sql.append("WHERE NA.COD_V_ELEMENTO = DVL.COD_N_MERCA||'-'||OE.COD_V_ALMACEN ");
			sql.append("AND NA.COD_N_DECLARACION_VALOR = DV.COD_N_DECLARACION_VALOR ");
			sql.append("AND NA.NUM_ANYO = DV.NUM_ANYO ");
			sql.append("AND NA.COD_N_VERSION = DV.COD_N_VERSION ");
